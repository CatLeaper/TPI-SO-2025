...
tiempo = 0
cambio = False
procesos = []  # FIX: inicializar la variable global
...
def vistaPrevia():
    ...
    if procesos == 1:
        ...
        # FIX: inicializar campos internos del simulador
        for p in procesos:
            p["estado"] = None
...
def agregarNuevosEjec() -> None:
    global tiempo
    # FIX: no cortar por TA > tiempo; recorrer todos
    for proceso in procesos:
...
def avanzarTiempo() -> None:
    ...
    if proceso_en_ejecucion:
        ...
        if proceso_en_ejecucion["tiempoRestante"] == 0:
            ...
            proceso_en_ejecucion["tiempoFinalizacion"] = tiempo   # FIX
...
def simulador() -> None:
    ...
    while True:
        ...
        if cambio:          # FIX: resetear el flag
            mostrarTablas()
            cambio = False
